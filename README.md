...
大A竞技场选股工具是一个面向低频研究的 A 股日报、收盘复盘、周度校准和产业链分析仓库。它把新闻事件、主线板块、资金方向、候选股票、量价确认、复盘回测和行业上下游研究沉淀成可审计的本地归档。
...
- 一键选股会优先使用本地 `a-data/` 与 `local/` 归档；候选股历史行情缺失或过期时，只增量补齐缺失尾部，不重复全量拉取。
- 当请求日期缺少完整日报主线时，综合选股会尝试使用同日 `close_review.json` 作为市场主线、资金、量能和命中偏差的兜底上下文；如果日报和收盘复盘都缺失，报告会明确标出市场状态可能滞后。
...
- `../a-data/stock_list.csv`：全 A 股票快照，供问财趋势池和历史更新 runner 读取。
- `../a-data/hist/<code>.csv`：个股未复权日线历史。选股、问财趋势池和 Vibe 的 A 股 alpha bench 都会优先读取这里。
- `local/YYYY-MM-DD/assembled.json`：日报主线、候选股和归档证据。
- `local/YYYY-MM-DD/close_review.json`：当日收盘复盘；在当日日报主线缺失时会作为一键选股的市场上下文兜底。

本仓库不会为了选股反复全量抓取历史行情。`scripts/local_hist.py` 会先读 `a-data/hist`，只有本地缺少目标日期尾部时才通过 AkShare 增量补齐，并写回同一个 CSV；`scripts/ensure_fresh.py` 是执行时新鲜度闸门，默认只检查本轮候选股。常用开关：

| 变量 | 是否必需 | 用途 |
| --- | --- | --- |
| `A_STOCK_DATA_DIR` | 否 | 覆盖默认 `../a-data` 数据目录。 |
| `A_STOCK_SELECTION_TODAY` | 否 | 调试/回放一键选股日期，未设置时使用系统日期。 |
| `A_STOCK_OFFLINE=1` | 否 | `china-stock-analysis` 深度分析中禁止联网补历史价。 |
| `A_STOCK_NO_CACHE=1` | 否 | `china-stock-analysis` 深度分析中关闭 `.cache` 写盘。 |

外部接口和密钥按需启用，缺失时应降级到本地归档、公开数据或手工导出结果：

| 能力 | 环境变量 | 必需性与说明 |
| --- | --- | --- |
| 问财趋势股票池 `iwencai-trend-stock-pool` | 无 | 默认不需要问财 key。脚本用 `a-data` + AkShare 公开数据近似问财条件，主力净流入等专有字段用成交额、换手率、量能和趋势代理。 |
| 问财/同花顺自然语言接口 | `IWENCAI_API_KEY`，可选 `IWENCAI_BASE_URL` | 仅 `a-stock-data` 或需要真实问财 OpenAPI/pywencai 语义搜索时使用；未配置时不阻塞一键选股。 |
| Vibe-Trading `iwencai_search` MCP 工具 | `VIBE_TRADING_IWENCAI_KEY` | 只影响 Vibe 后端里的自然语言 A 股研究搜索；与本仓库默认问财趋势池不是同一个变量。 |
| Vibe-Trading 投委会 / swarm | `LANGCHAIN_PROVIDER`、`LANGCHAIN_MODEL_NAME`、对应 provider key，例如 `OPENROUTER_API_KEY` 或 `OPENAI_API_KEY` | 只有选择“Vibe 优先”投委会时需要。默认本地投委会规则不需要 LLM key。 |
| Vibe-Trading A 股 alpha bench | `VIBE_TRADING_A_DATA_DIR` 或 `A_DATA_DIR` | 覆盖 Vibe 读取的本地 `a-data` 目录；默认会向上查找仓库同级 `a-data`。 |
| Tushare | `TUSHARE_TOKEN` | 可选。Vibe 的 CSI300 alpha bench 在找不到本地 `a-data` 时会用它兜底；本仓库一键选股默认不依赖它。 |
| Vibe 历史数据缓存 | `VIBE_TRADING_DATA_CACHE=1` | 可选。开启后 Vibe loader 会把已结算的历史数据缓存到用户目录，提升重复回测命中率。 |
| iTick / Zhitu / FRED / 美股数据 | `ITICK_API_TOKEN`、`ZHITU_API_TOKEN`、`FRED_API_KEY`、`FINNHUB_API_KEY`、`ALPHAVANTAGE_API_KEY`、`TIINGO_API_KEY`、`FMP_API_KEY` | 可选备用源或宏观/美股数据源。缺失时相关检查会标记为 `missing_credentials`，不应影响本地 A 股选股主流程。 |
...
一键选股的日期处理会显式写入报告：`requested_date` 是用户请求日期，`archive_date` 是实际使用的日报归档日期，`source` 会标识 `archive`、`latest_archive_fallback` 或 `close_review_fallback`。如果请求日没有 `assembled.json`，但有同日 `close_review.json`，系统会用收盘复盘里的主线、板块命中和个股命中对候选做小幅校准；如果两者都没有，报告会给出“当前市场状态可能滞后”的警告。
...
综合选股默认启用执行时数据新鲜度闸门：先收敛候选股，再检查这些候选在 `a-data/hist` 里的最新日期；缺失或过期时只增量补尾部。需要严格离线时使用 `--no-fresh-backfill`，需要完全跳过新鲜度检查时使用 `--skip-fresh-check`。
