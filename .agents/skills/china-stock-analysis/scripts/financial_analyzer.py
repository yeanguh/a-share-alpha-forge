...
class FinancialAnalyzer:
    ...
    def analyze_solvency(self) -> Dict:
        ...
        if debt_ratio is not None and debt_ratio > 70:
            risk_checks.append(f"资产负债率偏高 ({debt_ratio:.1f}%)，需关注偿债压力")
        if current_ratio is not None and current_ratio < 1:
            risk_checks.append(f"流动比率偏低 ({current_ratio:.2f})，短期偿债能力较弱")
        if quick_ratio is not None and quick_ratio < 0.8:
            risk_checks.append(f"速动比率偏低 ({quick_ratio:.2f})，短期流动性风险")
        result["risks"] = risk_checks
    ...
    def analyze_operation(self) -> Dict:
        ...
        if ar_days is not None and ar_days > 90:
            observation_checks.append(f"应收账款周转天数较长 ({ar_days:.0f}天)，回款较慢")
        if inventory_days is not None and inventory_days > 180:
            observation_checks.append(f"存货周转天数较长 ({inventory_days:.0f}天)，库存管理需关注")
        if asset_turnover is not None and asset_turnover < 0.5:
            observation_checks.append(f"总资产周转率较低 ({asset_turnover:.2f})，资产利用效率有待提高")
        result["observations"] = observation_checks
