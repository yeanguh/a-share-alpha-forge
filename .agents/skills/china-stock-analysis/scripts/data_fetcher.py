...
def get_stock_info(code: str) -> dict:
    ...
    try:
        ...
        for _, row in df.iterrows():
            # akshare has changed this endpoint shape more than once; use the
            # first non-empty cell as the key and the last non-empty cell as the
            # value so the parser survives extra helper columns.
            values = [value for value in row.tolist() if value not in (None, "", "--")]
            if len(values) < 2:
                continue
