...
1. Start from the latest or requested `local/YYYY-MM-DD/assembled.json`.
   Prefer `eligible_beneficiaries`, then `leading_stocks`, then `stocks`.
2. Add theme evidence from `daily_mainlines` and `sector_rankings`.
3. Add industry and bottleneck evidence from `industry-analysis/*/source_data.json`.
...
4. Add quote, valuation, financial, and risk context only for the narrowed set.
   Use `china-stock-price-analysis` for quick quote/valuation and
   `china-stock-analysis` for deeper fundamentals.
5. Treat `a-stock-data` and Vibe-Trading as expandable data/factor toolboxes:
...
6. Output four lists: core pool, watchlist, reject/exclusion list, and missing
...
- Prefer archive-backed candidates over concept-only names.
